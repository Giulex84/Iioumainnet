...
export default async function handler(req,res){
  try{
    ...
    if(req.method==='POST'){
      ...
      const username=String(counterpartyUsername||'').trim().replace(/^@/,'');
      const numericAmount=Number(amount);
      const cleanNote=String(note||'').trim();
      if(!/^[A-Za-z0-9_-]{3,32}$/.test(username))return res.status(400).json({success:false,error:'Enter a valid Pi username'});
      if(username.toLowerCase()===user.username.toLowerCase())return res.status(400).json({success:false,error:'You cannot create an IOU with yourself'});
      ...
      if(dueDate){const d=new Date(`${dueDate}T00:00:00Z`);if(Number.isNaN(d.getTime()))return res.status(400).json({success:false,error:'Invalid due date'});parsedDueDate=dueDate}
      const known=await getUserByUsername(username);
